...
class GetExploreUseCase
    extends UseCase<List<NewsModel>, GetExploreUseCaseParams> {
  final FeedRemoteRepository feedRemoteRepository;

  const GetExploreUseCase({
    required this.feedRemoteRepository,
  });

  @override
  Future<Either<DataException, List<NewsModel>>> call(
    GetExploreUseCaseParams params,
  ) async {
    try {
      var response = await feedRemoteRepository.getExplore(
...
class GetExploreUseCaseParams {
  final int countryId;

  const GetExploreUseCaseParams({
